...
class Progress:
    ...
    def display(self):
        ...
        if now - self.start > 3:
            ...
            S = eta % 60
            if self.total:
                s = "%s %2d%% (ETA %02d:%02d:%02d)" % (
                    self.info,
                    self.num * 100.0 / self.total,
                    H,
                    M,
                    S,
                )
            ...
            s = "%s %2d%%" % (self.info, self.num * 100.0 / self.total)
        else:
...
class progressHTTPConnection(HTTPConnection):
...
def http_request(data, server, port, url):
    h = progressHTTPConnection(server, port)
...
def usage():
    print(
        """This program is to be used to upload files to the PyWeek system.
